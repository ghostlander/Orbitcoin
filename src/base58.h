...
/* P2PK and P2PKH addresses begin with 'o' */
const uchar PUBKEY_ADDRESS_PREFIX = 0x73;

/* P2SH addresses begin with 's' */
const uchar SCRIPT_ADDRESS_PREFIX = 0x7D;
...
inline std::string EncodeBase58(const uchar *pbegin, const uchar *pend) {
    CAutoBN_CTX pctx;
    CBigNum bn58 = 58;
    CBigNum bn0 = 0;
    // Convert big endian data to little endian
    // Extra zero at the end make sure bignum will interpret as a positive number
    std::vector<uchar> vchTmp(pend - pbegin + 1, 0);
    ...
    for(const uchar *p = pbegin; (p < pend) && (*p == 0); p++)
      str += pszBase58[0];
...
inline std::string EncodeBase58(const std::vector<uchar> &vch) {
    return(EncodeBase58(&vch[0], &vch[0] + vch.size()));
}

// Decode a base58-encoded string psz into byte vector vchRet
// returns true if decoding is successful
inline bool DecodeBase58(const char *psz, std::vector<uchar> &vchRet) {
    CAutoBN_CTX pctx;
    vchRet.clear();
    CBigNum bn58 = 58;
    CBigNum bn = 0;
    ...
    std::vector<uchar> vchTmp = bn.getvch();
...
// returns true if decoding is successful
inline bool DecodeBase58(const std::string& str, std::vector<uchar> &vchRet) {
    return(DecodeBase58(str.c_str(), vchRet));
}

// Encode a byte vector to a base58-encoded string, including checksum
inline std::string EncodeBase58Check(const std::vector<uchar> &vchIn) {
    // add 4-byte hash check to the end
    std::vector<unsigned char> vch(vchIn);
    uint256 hash = Hash(vch.begin(), vch.end());
    vch.insert(vch.end(), (uchar *) &hash, (uchar *) &hash + 4);
    return(EncodeBase58(vch));
}

// Decode a base58-encoded string psz that includes a checksum, into byte vector vchRet
// returns true if decoding is successful
inline bool DecodeBase58Check(const char *psz, std::vector<uchar> &vchRet) {
...
class CBase58Data {
  protected:
    ...
    void SetData(int nVersionIn, const uchar *pbegin, const uchar *pend) {
        SetData(nVersionIn, (void *) pbegin, pend - pbegin);
    }

  public:
    bool SetString(const char *psz) {
        std::vector<uchar> vchTemp;
    ...
    std::string ToString() const {
        std::vector<uchar> vch(1, nVersion);
 ...
 * The data vector for public-key-hash-addresses contains RIPEMD160(SHA256(pubkey)),
 * where pubkey is the serialised public key.
 * The data vector for script-hash-addresses contains RIPEMD160(SHA256(cscript)),
 * where cscript is the serialised redemption script. */
class CCoinAddress;
...
class CCoinSecret : public CBase58Data {
  public:
    ...
    bool IsValid() const {
        bool fExpectTestNet = false;
        switch(nVersion) {
            case(128 + CCoinAddress::PUBKEY_ADDRESS):
                break;
            case(128 + CCoinAddress::PUBKEY_ADDRESS_TEST):
                fExpectTestNet = true;
                break;
            default:
                return(false);
