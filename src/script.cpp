...
bool IsCanonicalSignature(const valtype &vchSig) {
    ...
    if((ulong)(nLenR + nLenS + 7) != vchSig.size())
      return(error("Non-canonical signature: R+S length mismatch"));

    const uchar *R = &vchSig[4];
    if(R[-2] != 0x02)
...
bool EvalScript(vector<vector<uchar> > &stack, const CScript &script, const CTransaction &txTo,
  uint nIn, uint flags, int nHashType) {
    ...
    if(script.size() > 10000) return(false);
        ...
        while(pc < pend) {
            ...
            if(fExec && (0 <= opcode) && (opcode <= OP_PUSHDATA4)) {
                ...
                case(OP_CHECKLOCKTIMEVERIFY): {
                    ...
                    if(nLockTime > txTo.nLockTime) return(false);
                ...
                case(OP_ROLL): {
                    ...
                    popstack(stack);
                    if((n < 0) || (n >= (int)stack.size())) return(false);
                ...
                case(OP_CAT): {
                    ...
                    popstack(stack);
                    if(stacktop(-1).size() > 520) return(false);
                } break;
                ...
                case(OP_SUBSTR): {
                    ...
                    if((nBegin < 0) || (nEnd < nBegin)) return(false);
                    if(nBegin > (int)vch.size()) nBegin = vch.size();
                    if(nEnd > (int)vch.size()) nEnd = vch.size();
                ...
                case(OP_RIGHT): {
                    ...
                    if(nSize > (int)vch.size()) nSize = vch.size();
                    if(opcode == OP_LEFT) vch.erase(vch.begin() + nSize, vch.end());
                    else vch.erase(vch.begin(), vch.end() - nSize);
                    popstack(stack);
                } break;
                ...
                case(OP_SIZE): {
                    // (in -- in size)
                    if(stack.size() < 1) return(false);
                    CBigNum bn((int)stacktop(-1).size());
                    stack.push_back(bn.getvch());
                } break;
                ...
                case(OP_XOR): {
                    ...
                    uint i;
                    if(opcode == OP_AND) {
                        for(i = 0; i < vch1.size(); i++)
                          vch1[i] &= vch2[i];
                    } else if(opcode == OP_OR) {
                        for(i = 0; i < vch1.size(); i++)
                          vch1[i] |= vch2[i];
                    } else if(opcode == OP_XOR) {
                        for(uint i = 0; i < vch1.size(); i++)
                          vch1[i] ^= vch2[i];
                    }
                    popstack(stack);
                } break;
                ...
                case(OP_HASH256): {
                    ...
                    valtype vchHash(((opcode == OP_RIPEMD160) ||
                      (opcode == OP_SHA1) || (opcode == OP_HASH160)) ? 20 : 32);
                ...
                case(OP_CHECKMULTISIGVERIFY): {
                    ...
                    if((int)stack.size() < i) return(false);

                    int nKeysCount = CastToBigNum(stacktop(-i)).getint();

                    if((nKeysCount < 0) || (nKeysCount > 20)) return(false);

                    nOpCount += nKeysCount;

                    if(nOpCount > 201) return(false);
                    ...
                    if(nSigsCount < 0 || nSigsCount > nKeysCount) return(false);
                    ...
                    for(j = 0; j < nSigsCount; j++) {
                        valtype &vchSig = stacktop(-isig - j);
                        scriptCode.FindAndDelete(CScript(vchSig));
                    }
                    ...
                    while(fSuccess && (nSigsCount > 0)) {
                        ...
                        if(fOk) fOk = CheckSig(vchSig, vchPubKey, scriptCode, txTo, nIn, nHashType);
...
uint256 SignatureHash(CScript scriptCode, const CTransaction &txTo, uint nIn,
  int nHashType) {
    uint i;
...
private:
    ...
    typedef boost::tuple<uint256, std::vector<uchar>, std::vector<uchar> > sigdata_type;
    std::set< sigdata_type> setValid;
    CCriticalSection cs_sigcache;

public:
    bool Get(uint256 hash, const std::vector<uchar> &vchSig,
      const std::vector<uchar> &pubKey) {
    ...
    void Set(uint256 hash, const std::vector<uchar> &vchSig,
      const std::vector<uchar> &pubKey) {
...
bool CheckSig(vector<uchar> vchSig, vector<uchar> vchPubKey, CScript scriptCode,
  const CTransaction &txTo, uint nIn, int nHashType) {
    ...
    if(!nHashType) {
        ...
        if(nHashType != vchSig.back()) return(false);
    }
    vchSig.pop_back();
...
bool Solver(const CScript &scriptPubKey, txnouttype &typeRet,
  vector<vector<uchar> > &vSolutionsRet) {
    ...
    if(mTemplates.empty()) {
        ...
        /* Address tx, sender provides hash of pubkey, receiver provides signature and pubkey */
    ...
    if(scriptPubKey.IsPayToScriptHash()) {
        ...
        vector<uchar> hashBytes(scriptPubKey.begin() + 2, scriptPubKey.begin() + 22);
        ...
        while(true) {
            if((pc1 == script1.end()) && (pc2 == script2.end())) {
                ...
                if(typeRet == TX_MULTISIG) {
                    ...
                    uchar m = vSolutionsRet.front()[0];
                    uchar n = vSolutionsRet.back()[0];
...
bool SignN(const vector<valtype> &multisigdata, const CKeyStore &keystore,
  uint256 hash, int nHashType, CScript &scriptSigRet) {
    int nSigned = 0;
    int nRequired = multisigdata.front()[0];
    uint i;

    for(i = 1; i < (multisigdata.size() - 1) && (nSigned < nRequired); i++) {
...
bool IsStandard(const CScript &scriptPubKey) {
    ...
    if(whichType == TX_MULTISIG) {
        uchar m = vSolutions.front()[0];
        uchar n = vSolutions.back()[0];
...
uint HaveKeys(const vector<valtype>& pubkeys, const CKeyStore &keystore) {
    uint nResult = 0;
...
bool VerifyScript(const CScript &scriptSig, const CScript &scriptPubKey,
  const CTransaction &txTo, uint nIn, uint flags, int nHashType) {
    vector<vector<uchar> > stack, stackCopy;
...
bool SignSignature(const CKeyStore &keystore, const CScript &fromPubKey,
  CTransaction &txTo, uint nIn, int nHashType) {
    ...
    if(whichType == TX_SCRIPTHASH) {
        ...
        txnouttype subType;
        bool fSolved = Solver(keystore, subscript, hash2, nHashType,
          txin.scriptSig, subType) && (subType != TX_SCRIPTHASH);
...
static CScript CombineMultisig(CScript scriptPubKey, const CTransaction& txTo,
  uint nIn, const vector<valtype> &vSolutions, vector<valtype> &sigs1, vector<valtype> &sigs2) {
    uint i;
    ...
    uint nSigsRequired = vSolutions.front()[0];
    uint nPubKeys = vSolutions.size() - 2;
    ...
    for(i = 0; (i < nPubKeys) && (nSigsHave < nSigsRequired); i++) {
    ...
    for(i = nSigsHave; i < nSigsRequired; i++)
      result << OP_0;

    return(result);
}

static CScript CombineSignatures(CScript scriptPubKey, const CTransaction &txTo,
  uint nIn, const txnouttype txType, const vector<valtype> &vSolutions,
    ...
    switch (txType) {
        ...
        case(TX_SCRIPTHASH):
            if(sigs1.empty() || sigs1.back().empty()) {
                ...
                txnouttype txType2;
                vector<vector<uchar> > vSolutions2;
                Solver(pubKey2, txType2, vSolutions2);
                sigs1.pop_back();
                sigs2.pop_back();
                CScript result = CombineSignatures(pubKey2, txTo, nIn, txType2,
...
CScript CombineSignatures(CScript scriptPubKey, const CTransaction &txTo,
  uint nIn, const CScript &scriptSig1, const CScript &scriptSig2) {
    ...
    txnouttype txType;
    vector<vector<uchar> > vSolutions;
...
uint CScript::GetSigOpCount(bool fAccurate) const {
    uint n = 0;
...
uint CScript::GetSigOpCount(const CScript &scriptSig) const {
...
void CScript::SetDestination(const CTxDestination &addr) {
    boost::apply_visitor(CScriptVisitor(this), addr);
}
...
bool CScriptCompressor::IsToScriptID(CScriptID &hash) const {
    ...
    if((script.size() == 23) && (script[0] == OP_HASH160) &&
      (script[1] == 20) && (script[22] == OP_EQUAL)) {
...
bool CScriptCompressor::IsToPubKey(std::vector<unsigned char> &pubkey) const {
    ...
    if((script.size() == 67) && (script[0] == 65) && (script[66] == OP_CHECKSIG) &&
      (script[1] == 0x04)) {
        ...
        // SetPubKey fails if this is not a valid public key, a case that would not be compressible
        return(key.SetPubKey(CPubKey(pubkey)));
    }

    return(false);
}

bool CScriptCompressor::Compress(std::vector<uchar> &out) const {
...
uint CScriptCompressor::GetSpecialSize(uint nSize) const {
    if((nSize == 0) || (nSize == 1))
      return(20);
...
bool CScriptCompressor::Decompress(uint nSize, const std::vector<uchar> &in) {
...
const map<uchar, string> mapSigHashTypes = boost::assign::map_list_of
  ...
  (static_cast<uchar>(SIGHASH_SINGLE|SIGHASH_ANYONECANPAY), string("SINGLE|ANYONECANPAY"));
