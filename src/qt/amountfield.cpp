...
void AmountField::setText(const QString &text) {
    if (text.isEmpty())
        amount->clear();
    else
...
bool AmountField::validate() {
    ...
    if(valid && !CoinUnits::parse(currentUnit, text(), 0))
      valid = false;
...
QString AmountField::text() const {
...
QWidget *AmountField::setupTabChain(QWidget *prev) {
    QWidget::setTabOrder(prev, amount);
    return amount;
}

qint64 AmountField::value(bool *valid_out) const {
    qint64 val_out = 0;
...
void AmountField::setValue(qint64 value) {
    setText(CoinUnits::format(currentUnit, value));
}

void AmountField::unitChanged(int idx) {
    ...
    amount->setMaximum(qPow(10, CoinUnits::amountDigits(currentUnit)) -
